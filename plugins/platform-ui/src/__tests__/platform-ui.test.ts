...
describe('session', () => {
  ...
  it('should build ClassUIModel', async () => {
    ...
    const classModel = await uiPlugin.getClassModel(await session.getInstance(core.class.Class, contact.class.Contact))
  ...
  it('should build AttrUIModel without strings loaded', async () => {
    ...
    const session = coreService.newSession()
    const uiPlugin = await platform.getPlugin(ui.id)
    const ownModel = await uiPlugin.getOwnAttrModel(await session.getInstance(core.class.Class, contact.class.Contact))

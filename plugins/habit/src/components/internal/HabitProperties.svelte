<script type="ts">
  ...
  import habit, { Habit, HabitFieldType, HabitFieldValue } from '../..'
  ...
  export let object: Habit

  const coreService = getCoreService()

  let fieldValues: Map<HabitFieldType, HabitFieldValue[]> = new Map()

  let status: HabitFieldValue | undefined

  // Load and subscribe to any habit status values.
  createLiveQuery(habit.class.HabitFieldValue, {}, (docs) => {
    const fValues: Map<HabitFieldType, HabitFieldValue[]> = new Map()
    for (const d of docs) {
  ...
  let habitShortId: ShortID

  $: {
    let sv = fieldValues.get(HabitFieldType.Status) || []
    let acts: Action[] = []
    for (const s of sv) {
      if (!status) {
        status = s
    ...
    statusActions = acts
    coreService.then(cs => {
      habitShortId = cs.getModel().as(object, CORE_MIXIN_SHORTID)
    })
  }
</script>
...
<div class="habitContent">
  <div class="caption caption-1">
    <InlineEdit id="create_habit__input__name" bind:value={object.title} width="100%"
                label="Name" placeholder="Name"
                on:change={() => {coreService.update(object, null, {title: object.title})}} />
  </div>
  <div class="habitStatusBar">
    <div class="habitName">
      {#if habitShortId}
        {habitShortId.shortId}
      {/if}
    </div>
    {#if status}

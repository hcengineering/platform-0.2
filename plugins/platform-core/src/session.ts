...
export class TSession implements Session {
  ...
  constructor (platform: Platform, memdb: Db) {
    ...
    this.sessionProto = {
      getSession: () => this,
      __mapKey (): string { throw new Error('object model must override.') }
    }
  }
  ...
  instantiateSync<T extends Obj> (_class: Ref<Class<T>>, __layout: any): T {
    ...
    if (!proto) { throw new Error('instanceSync failed, no prototype for class: ' + _class) }
  ...
  async as<T extends Doc> (doc: Layout<Doc>, _class: Ref<Class<T>>): Promise<T | undefined> {
    ...
    if (classes && classes.includes(_class)) { return this.instantiate(_class, layout) }
  }

...
describe('server', () => {
  it('Check reference output', async () => {
    ...
    let msg = serializeMessageMarkdown(JSON.parse(t1) as MessageNode)
  ...
  it('check list with bold and italic', async () => {
    let msg = serializeMessageMarkdown(
      JSON.parse(
  ...
  it('Check parsing', async () => {
    const t1 =
      'Hello [Page1](ref://chunter.Page#5f8043dc1b592de172c26181) and [Page3](ref://Page#)'
    let msg = parseMessageMarkdown(t1)
    console.log(msg)
    expect(msg.type).toEqual('doc')

    let md = serializeMessageMarkdown(msg)

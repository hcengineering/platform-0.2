...
This is the code and content for <https://anticrm.org>.
...
Note that this will leave the platform-build tool in full-symlink mode.  To return to the default isolated
...
```bash
npm run hugo -- new docs/concepts/platform-mixin.md
```

### Blog post

```bash
npm run hugo -- new posts/platform-everywhere.md
```
